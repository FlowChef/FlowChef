...
class StableDiffusionPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Check if operator is provided for inverse problem
        if operator is not None:
            # Prepare for inverse problem
            ...
            if input_image is not None:
                ...
                if save_masked_image and output_path is not None:
                ...
                # Create random latents for unmasked regions
            ...
            # Set optimization parameters if provided
            optimization_steps = optimization_steps if optimization_steps is not None else 1
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Apply inverse problem optimization if operator is provided
                if operator is not None and input_image is not None and i <= max_steps:
                    # Optimize latents for inverse problem
                    with torch.enable_grad():
                        latents_opt = torch.autograd.Variable(latents.detach().clone(), requires_grad=True)
                        optimizer = torch.optim.SGD([latents_opt], lr=learning_rate)

                        for _ in range(optimization_steps):
                            # Get predicted x_0
                            alpha_t = self.scheduler.alphas_cumprod[t.item()]
                            latents_p = (latents_opt - (1 - alpha_t).sqrt() * noise_pred) / alpha_t.sqrt()
                            # Clip latents to the range [-1, 1] to ensure stability
                            # latents_p = torch.clamp(latents_p, -1.0, 1.0)
                            # print(alpha_t)

                            if operator.name == "inpainting":
                                loss = (100 * torch.nn.functional.mse_loss(latents_p, masked_latents, reduction='none') * mask).mean()
                            else:
                                ...
                                loss = 100 * torch.nn.functional.mse_loss(downsampled, noisy_image.to(downsampled.dtype))

                            loss.backward()
                            # Calculate and print gradient norm and loss
                            # grad_norm = torch.norm(latents_opt.grad.data)
                            # print(f"Gradient norm: {grad_norm.item()}, Loss: {loss.item()}")
                            optimizer.step()
                            optimizer.zero_grad()

                    latents = latents_opt.detach()
    ...
    def optimize_latents(self, latents, v_pred, t, device, optimization_steps, learning_rate,
        ...
        with torch.enable_grad():
            ...
            for _ in range(optimization_steps):
                alpha_t = self.scheduler.alphas_cumprod[t.item()]
                latents_p = (latents - (1 - alpha_t).sqrt() * v_pred) / alpha_t.sqrt()
