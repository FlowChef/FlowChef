...
class RectifiedFlowPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        mask = operator.get_mask((1, 1, height, width)).to(device)


        # 4. Preprocess image
        image = downsample(self.image_processor.preprocess(input_image))
        ...
        upsampled_latents = retrieve_latents(self.vae.encode(upsampled_image), generator=generator) * self.vae.config.scaling_factor
        ...
        mask = TF.Resize((h, w))(mask.to(device).squeeze(0))
        mask = (mask > 0.5).float()
        ...
        if save_masked_image:
            masked_image = self.vae.decode(upsampled_latents / self.vae.config.scaling_factor, return_dict=False)[0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                latents = self.perform_denoising_step(
                    ...
                    max_steps, timesteps, mask, masked_latents, noisy_image, downsample, upsample, operator
                )
    ...
    def perform_denoising_step(self, latents, t, prompt_embeds, do_classifier_free_guidance, guidance_scale,
        ...
        if step <= max_steps:
            latents = self.optimize_latents(latents, v_pred, t,
                                            device, optimization_steps, learning_rate, mask, masked_latents, noisy_image, downsample, upsample, operator)
    ...
    def optimize_latents(self, latents, v_pred, t, device, optimization_steps, learning_rate,
                         mask, masked_latents, noisy_image, downsample, upsample, operator):
        with torch.enable_grad():
            ...
            for _ in range(optimization_steps):
                ...
                if operator.name=="inpainting":
                    """Note:
                    We could use the VAE model for inpainting however as degradation fuction is known and does not affect as long as gaussian noise is zero. 
                    Alternatively, we could easily move this to else condition. But importantly, this allows us to connect with the image editing which happens in the VAE latent space.
                    """
                    loss = (0.001*torch.nn.functional.mse_loss(latents_p, masked_latents, reduction='none')*mask).mean()
                else:
                    decoded_latents = self.vae.decode(latents_p / self.vae.config.scaling_factor, return_dict=False)[0]
                    downsampled = downsample(decoded_latents)
                    loss = 1000*torch.nn.functional.mse_loss(downsampled, noisy_image.to(downsampled.dtype))
