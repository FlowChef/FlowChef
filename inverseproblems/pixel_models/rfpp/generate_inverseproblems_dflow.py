...
def sample_ode_generative(model, z1, N, use_tqdm=True, solver='euler', label=None, inversion=False, 
    ...
    # z_inv = np.sqrt(1-0.1) * z_inv + np.sqrt(0.1) * torch.randn_like(z_inv)
    
    torch.manual_seed(42)  # Fix random seed
    # z_inv = 0.95 * z_inv + np.sqrt(1 - 0.95) * torch.randn_like(z_inv)
    z_inv = 0.95 * z_inv + 0.05 * torch.randn_like(z_inv)
    # z_inv = np.sqrt(0.95) * z_inv + np.sqrt(1 - 0.95) * torch.randn_like(z_inv)
    ...
    with torch.enable_grad():
        ...
        for iteration in tqdm(range(50)):
            def closure():
                optim_img.zero_grad()
                out = denoising(z_inv)
                # reg = - torch.clamp((z_inv**2).sum([1,2,3])*0.5, min=-1e6, max=1e6) + (d - 1) * torch.log(torch.sqrt(torch.sum(z_inv**2, dim=(1, 2, 3))) + 1e-5)
                loss = (torch.sum((downsample(out) - downsampled)**2, dim=(1,2,3))).sum()
                # loss = (torch.sum((downsample(out) - downsampled)**2, dim=(1,2,3)) + reg).sum()
                loss.backward()
                return loss
