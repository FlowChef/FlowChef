...
# N=20 ablation experiments for both datasets in parallel
for dataset in "AFHQ-Cat" "CelebA"; do
    ...
    for mask_size in 20 30; do
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dflow.py --gpu 0 \
                    --solver euler --N 20 --sampler new --batchsize 1 \
                    --ckpt ./checkpoints/${ckpt} \
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dflow/inverseproblems/N20_${dataset,,}_box_inpaint_m${mask_size}_n${noise} \
        ...
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dflow.py --gpu 0 \
                    ...
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dflow/inverseproblems/N20_${dataset,,}_super_resolution_s${scale}_n${noise} \
        ...
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dflow.py --gpu 0 \
                    ...
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dflow/inverseproblems/N20_${dataset,,}_deblur_k${kernel}_n${noise} \
...
# CUDA_VISIBLE_DEVICES=0 python generate_inverseproblems_dflow.py --gpu 0 \
#     --solver euler --N 20 --sampler new --batchsize 1 \
#     --ckpt ./checkpoints/afhq-configF.pth \
#     --config configs_unet/afhq64_ve_aug.json \
#     --input_dir ./data/AFHQ-Cat \
#     --dir outputs/dflow/inverseproblems/test_run \
#     --inverse_problem "super_resolution" --noise_sigma 0.0 \
#     --scale_factor 4 --gradient_scale 500
