...
# N=100 baseline experiments for AFHQ-Cat and CelebA in parallel
for dataset in "AFHQ-Cat" "CelebA"; do
    for problem in "box_inpaint" "super_resolution" "deblur"; do
        ...
        (
            CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dps.py --gpu 0 \
                --solver euler --N 100 --sampler new --batchsize 1 \
                --ckpt ./checkpoints/${ckpt} \
                --config configs_unet/${config} \
                --input_dir ./data/${dataset} \
                --dir outputs/dps_org/inverseproblems/N100_${dataset,,}_${problem} \
...
# N=200 ablation experiments for both datasets in parallel
for dataset in "AFHQ-Cat" "CelebA"; do
    # Set dataset-specific checkpoints and configs
    if [ "$dataset" = "AFHQ-Cat" ]; then
        ckpt="afhq-configF.pth"
    ...
    for mask_size in 20 30; do
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dps.py --gpu 0 \
                    --solver euler --N 200 --sampler new --batchsize 1 \
                    --ckpt ./checkpoints/${ckpt} \
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dps_org/inverseproblems/N200_${dataset,,}_box_inpaint_m${mask_size}_n${noise} \
                    --inverse_problem "box_inpaint" --noise_sigma ${noise} \
                    --mask_size ${mask_size} --gradient_scale 50 ;
                rm -f "/tmp/gpu_lock_${gpu}"
            ) &
        done
    done
        ...
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dps.py --gpu 0 \
                    ...
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dps_org/inverseproblems/N200_${dataset,,}_super_resolution_s${scale}_n${noise} \
                    --inverse_problem "super_resolution" --noise_sigma ${noise} \
                    --scale_factor ${scale} --gradient_scale 50 ;
                rm -f "/tmp/gpu_lock_${gpu}"
            ) &
        done
    done
        ...
        for noise in 0.0 0.05; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python generate_inverseproblems_dps.py --gpu 0 \
                    ...
                    --config configs_unet/${config} \
                    --input_dir ./data/${dataset} \
                    --dir outputs/dps_org/inverseproblems/N200_${dataset,,}_deblur_k${kernel}_n${noise} \
                    --inverse_problem "deblur" --noise_sigma ${noise} \
                    --kernel_size 11 --blur_sigma ${kernel} --gradient_scale 50 ;
                rm -f "/tmp/gpu_lock_${gpu}"
            ) &
        done
    done
...
# echo "Running example inpainting command..."
# CUDA_VISIBLE_DEVICES=0 python generate_inverseproblems_dps.py \
#     --gpu 0 \
#     --solver euler \
#     --N 200 \
#     --sampler new \
...
#     --config ./rfpp/configs_unet/afhq64_ve_aug.json \
#     --input_dir ./data/AFHQ-Cat \
#     --dir outputs/test_dps \
#     --inverse_problem "box_inpaint" \
#     --noise_sigma 0.0 \
#     --mask_size 32 \
#     --gradient_scale 100
