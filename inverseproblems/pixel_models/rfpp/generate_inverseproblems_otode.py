...
def get_args():
    ...
    # ot-ode arguments
    parser.add_argument('--sigma_noise', type=float, default=0.0000001, help='Noise sigma for OT-ODE')
    parser.add_argument('--start_time', type=float, default=0.2, help='Start time for OT-ODE between 0-1')
...
def sample_ode_generative(model, z1, N, use_tqdm=True, solver='euler', label=None, inversion=False, 
    ...
    start_i = int(steps*arg.start_time)
    ...
    z = upsampled*(1-time_schedule[start_i]) + torch.randn_like(z) * (time_schedule[start_i])

    # Track max GPU memory usage
    torch.cuda.reset_peak_memory_stats()
    start_time = time.time()
    
    for i in tq(range(start_i, steps)):
        with torch.no_grad():
            time_i = torch.ones((batchsize), device=z.device) * time_schedule[i]
            vt = model(z, time_i, label)
            rt_squared = (time_schedule[i]**2) / (time_schedule[i]**2 + (1-time_schedule[i])**2)
            x0hat = z - time_schedule[i] * vt
            d = (downsampled - downsample(x0hat))
            sol = torch.zeros_like(d)
            for k in range(d.shape[0]):
                sol_tmp = 1 / (downsample(torch.ones_like(z))[k] * rt_squared + sigma_noise**2) * d[k]
                sol[k] = sol_tmp.reshape(d[k].shape)
            vec = upsample(sol)
        g = torch.autograd.functional.vjp(
            lambda x: model(x, time_i, label), inputs=z, v=vec)[1]

        with torch.no_grad():
            g = vec + time_schedule[i] * g
            ratio = time_schedule[i] / (1-time_schedule[i])
            v_adapted = vt - ratio * g
            if i > 0:
                dt = time_schedule[i] - time_schedule[i-1]
            else:
                dt = time_schedule[i]
